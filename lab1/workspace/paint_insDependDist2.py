...
if filename == 'auto':
    ...
    commands = [f.replace('insDependDist2-', '').replace('.csv', '').replace('./', '') for f in files]
    SAVE_FILENAME = "imgs/insDependDist2-all.png"
    # print("commands:", commands)
    for i, command in enumerate(commands):
        process(filename=f"insDependDist2-{command}.csv", color=colors[i % len(colors)], marker=marks[i % len(marks)], line=lines[i % len(lines)])
    plt.legend(commands)
else:
    process(filename)
