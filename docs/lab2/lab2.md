...
| -------------- | ------- | ------- | ------- | --------- |
| BHT 预测器     | 94.866  | 94.531  | 94.9518 | 94.9034   |
| 全局历史预测器 | 97.4675 | 97.4281 | 97.4714 | 97.4927   |
| 锦标赛预测器   | 94.6229 | 96.994  | 97.068  | 94.6771   |
| TAGE 预测器    | 95.0842 | 95.862  | 95.9615 | 97.085    |
...
| --------------------------------------------------- | ---------- | ---------- | ---------- | ---------- |
| BHT: `entry_log=17`、`scnt_width=2`                 |            |            |            |            |
| GH: `gw=20`, `e=17`, `sw=2`, `hash<20>`             | `fold_xor` | `fold_xor` | `fold_xor` | `fold_xor` |
| Tournament: `GH.hash`, `e=10`                       | `fold_xor` | `fold_xor` | `fold_xor` | `fold_xor` |
| TAGE:`tn=5`, `T0_e=16`, `T1_gw=4`, `a=2`, `Tn_e=11` |            |            |            |            |



在是否跳转预测正确的情况下，跳转地址预测正确率：

| 预测器     | `bzip`  | `sjeng` | `wrf`   | `sphinx3` |
| ---------- | ------- | ------- | ------- | --------- |
| BHT        | 99.6786 | 99.6759 | 99.6834 | 97.4035   |
| Tournament | 99.0568 | 98.9993 | 99.0217 | 99.0146   |
...
![image-20221130234649913](lab2.assets/image-20221130234649913.png)

下图为在不同工作载荷下，全局历史预测器预测准确率与历史位宽的关系。

由于本实验中在全局历史预测器设定历史长度时，同时设置了哈希函数的折叠长度，造成本图中 GHR 长度较小时 (<10) 表现不太正常，这部分数据忽略不考虑。

由图像可知，最好 GHR 长度在 20 左右。
